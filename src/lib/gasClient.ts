...
export class GasClient {
  ...
  private static normalizeOrderPlanObject(raw: any, index: number): any {
    ...
    return {
      ...
      knitStart: GasClient.formatDateValue(getVal('knitStart', 'Knit Start', 'Knit Start Date', 'Planned Knit Start')),
      knitEnd: GasClient.formatDateValue(getVal('knitEnd', 'Knit End', 'Knit End Date', 'Planned Knit End')),
      target: Number(getVal('target', 'Target', 'Target Qty', 'Target (Kg)', 'Target(Kg)', 'Target_Kg')) || 0,
      targetNextMonth: Number(getVal('targetNextMonth', 'Target Next Month', 'Target  Next Month', 'Next Month Target', 'Target Next')) || 0,
      allocationStart: GasClient.formatDateValue(getVal('allocationStart', 'Allocation Start', 'Allocation Start Date')),
      allocationEnd: GasClient.formatDateValue(getVal('allocationEnd', 'Allocation End', 'Allocation End Date')),
      allocatedQty: Number(getVal('allocatedQty', 'Allocated Qty', 'Allocated QTY', 'Allocated Quantity')) || 0,
      allocatedBal: Number(getVal('allocatedBal', 'Allocated Bal', 'Allocated Bal.', 'Allocated Balance')) || 0,
      greyReq: Number(getVal('greyReq', 'Grey Req', 'GREY REQ.', 'Grey Requirement')) || 0,
      knitPro: Number(getVal('knitPro', 'Knit Pro', 'KNIT PRO.', 'Knitting Production', 'Knit Prod')) || 0,
      knitBal: Number(getVal('knitBal', 'Knit Bal', 'KNIT BAL.', 'Knit Balance', 'Knitting Balance')) || 0,
      aKnitStart: GasClient.formatDateValue(getVal('aKnitStart', 'Actual Knit Start', 'A.Knit Start', 'A. Knit Start')),
      lastProductionDate: GasClient.formatDateValue(getVal('lastProductionDate', 'Last Production Date', 'Last Prod Date', 'A. Knit End/Last Production Date')),
      avgProdDay: Number(getVal('avgProdDay', 'Avg Prod Day', 'Avg Prod/Day', 'Avg. Prod/Day', 'Avg Production / Day')) || 0,

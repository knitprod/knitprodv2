...
export default function ProductionEntryView({ floors, onSubmitEntry, currentUser }: ProductionEntryViewProps) {
  ...
  const handleSaveProduction = (e: React.FormEvent) => {
    ...
    const ledgerPayload: LedgerRecord = {
      ...
      runningMachine: runM,
      idleMachine: idleM,
      machineUtilization: parseFloat(((runM / totM) * 100).toFixed(1)),
      idleMachinePct: parseFloat(((idleM / totM) * 100).toFixed(1)),
      idleProduction: idleM * 260,
      efficiency: parseFloat(((totalProduction / tgt) * 100).toFixed(1)),
      ...
      capacityUtilization: parseFloat(((runM / totM) * 100).toFixed(1)),
      totalOperator: totOp,
      absent: absOp,
